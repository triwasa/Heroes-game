...
enum CreatureStatistic implements CreatureStatisticIf{
    ...
    // TEST CREATURES
    TEST("name",2,2,10,1,Range.closed(2,2),0,"for unit testing", "ground",false),



    // NECROPILIS FRACTION
    SKELETON("Skeleton",5,4,6,4,Range.closed(1,3),1,"Average lvl1 foot soldier, but always in huge numbers thanks to necromancy skill and skeleton transformer.", "ground",false),
    WALKING_DEAD("Walking Dead",5,5,15,3,Range.closed(2,3),2,"Basically its the same skeleton with more hit points. I prefer buying 2 skeletons instead.", "ground",false),
    WIGHT("Wight",7,7,18,5,Range.closed(3,5),3,"Regenerating ability is really good when fighting weak enemies, especially shooters.\nSpecial: top wight of the stack regenerates all lost damage in the beginning of each round", "ground",false),
    VAMPIRE("Vampire",10,9,30,6,Range.closed(5,8),4,"NOTHING compared to their upgraded brothers. Keep the population growing and recruit after the upgrade.\nSpecial: no enemy retaliation.", "flying",false),
    ...
    // CASTLE FRACTION
    PIKEMAN("Pikeman",4,5,10,4,Range.closed(1,3),1, "Toughest lvl1 unit, but a bit slow.", "ground", false),
    ...
    TITAN("Titan", 24,24,300,11,Range.closed(40,60), 7, "Special: immune to mind-affecting spells, no hand-to-hand penalty, do 150% damage to black dragons.",  "ground",true),
    ...
    PIT_LORD("Pit Lord", 13,13,45,7, Range.closed(13,17),5, "Special: Can resurrect dead allied stack as demons once per combat. Number of raised demons cannot be larger than the number of pit lords performing the resurrection. Max number of demons is determined by the total hit points of the target stack.", "ground", true),
    ...
    WYVERN_MONARCH("Wyvern Monarch", 14,14,70,11,Range.closed(18,22), 6, "Special: chance to poison living enemies (stack's maximum health will be reduced every turn, can be removed by cure spell). Costly upgrade raises the damage by 4 and adds a poison abbility which can be very useful to weaken a strong stack. Hit points are very low for level 6 upgrade, so use wyvern monarchs as an offensive unit to finish off the weaklings and to try to poison really strong stacks (better do that after they used their retaliation).", "flying", true),
    HYDRA("Hydra", 16,18,175,5,Range.closed(25,45), 7, "Special: attacks all adjacent hexes, no enemy retaliation. Costs no special resources and the special abbility is one of the greatest: hydras attack all 8 hexes around them without fear of retaliation! ", "ground", false),
    CHAOS_HYDRA("Chaos Hydra", 18,20,250,7, Range.closed(25,45), 7, "Special: attacks all adjacent hexes, no enemy retaliation. Extra 75 hit points, 2 attack and defence, but most importantly extra speed. Well, speed still remains the slowest among level 7 upgrades, but the strategic use is at it's best.", "ground", true),
    ...
    MEDUSA("Medusa", 9,9,25,5, Range.closed(6,8), 4, "Special: no hand-to-hand penalty, hand-to-hand attacks have 20% chance to turn enemies to stone: 3 rounds, take 50% damage, unstoned when attacked. Very good creature, would be a better deal than beholder if had more than 4 shots...", "ground",false),
    ...
    CreatureStatistic(String aName, int aAttack, int aArmor, int aMaxHp, int aMoveRange, Range<Integer> aDamage, int aTier, String aDescription, String aMovement, boolean aIsUpgraded) {
        name = aName;
        attack = aAttack;
        armor = aArmor;
        maxHp = aMaxHp;
